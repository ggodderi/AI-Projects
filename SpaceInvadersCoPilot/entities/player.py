...
class Player:
    ...
    def can_fire(self) -> bool:
        # Allow firing if cooldown is zero and fewer than 5 bullets are in flight
        return self.cooldown <= 0.0 and len(self.bullets) < 5
    ...
    def fire(self) -> Projectile | None:
        ...
        bullet = Projectile(bx, by, PLAYER_BULLET_WIDTH, PLAYER_BULLET_HEIGHT, PLAYER_BULLET_SPEED, GREEN)
        self.bullets.append(bullet)
        # Reduce cooldown for rapid fire (optional tweak)
        self.cooldown = max(PLAYER_COOLDOWN * 0.5, 0.12)
        return bullet
