...
class Game:
    ...
    def update_invaders(self):
        ...
        # Count down cooldown frames after a drop so we don't chain-drop
        if getattr(self, "drop_cooldown", None) is None:
            self.drop_cooldown = 0
        ...
        hit_wall = (will_hit_left or will_hit_right) and (self.drop_cooldown == 0)

        if hit_wall:
            # Step down exactly one row
            for inv in alive:
                inv.pos.y += INVADER_STEP_DOWN

            # Place formation just inside the wall to avoid immediate re-trigger
            if will_hit_left:
                shift = (left_limit - min_x) + max(3, speed*2)
            else:
                # negative shift (move left) to tuck in from the right edge
                shift = (right_limit - max_x) - max(3, speed*2)

            for inv in alive:
                inv.pos.x += shift

            # Reverse direction and set a brief cooldown
            self.inv_dir *= -1
            # Cooldown proportional to speed so fast levels still move away enough
            self.drop_cooldown = max(5, int(8 - min(5, speed)))  # ~5–8 frames

            # ground check after stepping down
            for inv in alive:
    ...
    def draw(self):
        ...
        # Congrats banner if new high score crossed
    ...
    def game_over_screen(self):
        ...
        self.screen.blit(hsline, hsline.get_rect(center=(SCREEN_W//2, SCREEN_H//2 + 40)))
        self.screen.blit(prompt, prompt.get_rect(center=(SCREEN_W//2, SCREEN_H//2 + 80)))
        pygame.display.flip()
