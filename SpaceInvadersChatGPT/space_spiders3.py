...
class Game:
    ...
    def update_invaders(self):
        ...
        # Proposed horizontal delta this frame
        dx = speed * self.inv_dir

        # Predict bounds after move to decide if we HIT the wall this frame
        will_hit_left = (min_x + dx) <= left_limit
        will_hit_right = (max_x + dx) >= right_limit
        hit_wall = will_hit_left or will_hit_right

        if hit_wall:
            # Step down exactly one row and reverse direction (classic behavior)
            for inv in alive:
                inv.pos.y += INVADER_STEP_DOWN

            # Nudge horizontally so next frame we're inside the bounds to avoid repeated drops
            if will_hit_left:
                shift = (left_limit - min_x) + 1
                for inv in alive:
                    inv.pos.x += shift
            elif will_hit_right:
                shift = (right_limit - max_x) - 1
                for inv in alive:
                    inv.pos.x += shift
