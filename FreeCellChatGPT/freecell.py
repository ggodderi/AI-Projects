...
class Tableau(Pile):
    ...
    def get_click_index(self, x, y) -> Optional[int]:
        ...
        for i, card in enumerate(self.cards):
            cx, cy = px, py + i * CARD_SPACING_Y
            rect = pygame.Rect(cx, cy, CARD_W, CARD_H)
            if rect.collidepoint(x, y):
                return i
        # If below last card but inside column width, select last
        if self.cards:
            last_y = py + (len(self.cards) - 1) * CARD_SPACING_Y
            rect = pygame.Rect(px, last_y, CARD_W, CARD_H)
...
class Game:
    ...
    def pile_at_point(self, x, y) -> Optional[Pile]:
        ...
        for t in self.tableau:
            # For tableau we want taller hit area
            tx, ty = t.pos
            h = max(CARD_H, CARD_SPACING_Y * (len(t.cards) - 1) + CARD_H)
    ...
    def start_drag(self, x, y):
        ...
        if isinstance(self.drag_src, Tableau) and self.drag_index >= 0:
            top_pos = (self.drag_src.pos[0], self.drag_src.pos[1] + self.drag_index * CARD_SPACING_Y)
        else:
