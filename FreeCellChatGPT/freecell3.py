...
HEADER_H = 46  # visible top band for overlapped cards (rank + suit/face)
CARD_RADIUS = 10
CARD_SPACING_X = 16
...
class Card:
    ...
    def _face_icon(self) -> str:
        # Use chess-like crowns to differentiate face cards in the visible band
        if self.rank == 'K':
            return '♚'
        if self.rank == 'Q':
            return '♛'
        if self.rank == 'J':
            return '♝'
        # For A or numbers, show suit as the icon
        return self.suit

    def draw(self, surface, x, y, highlight=False, shadow=False, mode: str = "full"):
        """Draw the card.
        mode = 'full' for entire card, 'peek' to draw only the top band that stays visible under overlap.
        """
        if mode == "peek":
            # Draw a shallow band with rank+mini face/suit so covered cards are still identifiable
            band_rect = (x, y, CARD_W, HEADER_H)
            # shadow for band
            if shadow:
                s = pygame.Surface((CARD_W, HEADER_H), pygame.SRCALPHA)
                pygame.draw.rect(s, (0, 0, 0, 70), (6, 6, CARD_W, HEADER_H), border_radius=CARD_RADIUS)
                surface.blit(s, (x - 6, y - 6))
            rounded_rect(surface, band_rect, WHITE, CARD_RADIUS)
            pygame.draw.rect(surface, LIGHT_GRAY, band_rect, width=2, border_radius=CARD_RADIUS)
            if highlight:
                pygame.draw.rect(surface, YELLOW, band_rect, width=3, border_radius=CARD_RADIUS)
            # contents of band
            color = self.color
            ...
            surface.blit(suit_surf, (x + 8, y + 6 + rank_surf.get_height() + 2))
            icon = self._face_icon()
            icon_surf = SUIT_FONT.render(icon, True, color)
            surface.blit(icon_surf, (x + CARD_W - icon_surf.get_width() - 10, y + (HEADER_H - icon_surf.get_height()) // 2))
            return
        ...
        # center icon uses face icon for J/Q/K; suit otherwise
        icon = self._face_icon()
        big_icon = SUIT_FONT.render(icon, True, color)
        surface.blit(big_icon, (x + CARD_W // 2 - big_icon.get_width() // 2, y + CARD_H // 2 - big_icon.get_height() // 2))
...
class FreeCell(Pile):
    ...
    def draw(self, surface):
        ...
        if not self.cards:
            ...
            rounded_rect(surface, (x, y, CARD_W, CARD_H), DARK_GREEN, CARD_RADIUS)
            pygame.draw.rect(surface, LIGHT_GRAY, (x, y, CARD_W, CARD_H), 2, border_radius=CARD_RADIUS)
        # stack: draw all but the top as 'peek' bands so suit+face are visible, top as full
        for i, card in enumerate(self.cards):
            cy = y + i * sp
            mode = 'full' if i == len(self.cards) - 1 else 'peek'
            card.draw(surface, x, cy, shadow=True, mode=mode)
        x, y = self.pos
        if not self.cards:
            # empty placeholder
            rounded_rect(surface, (x, y, CARD_W, CARD_H), DARK_GREEN, CARD_RADIUS)
